...
<template>
  ...
  <div v-if="!renderData.loading" class="form-container pretty-scrollbar">
    ...
    >
      ...
      <el-form-item label="其它标签" :prop="C_Fields.tagList">
        <simple-tag-list
      ...
      <el-form-item label="其它选项" :prop="C_Fields.optionList">
        <simple-tag-list
...
<script lang="ts" setup>
  ...
  const submitAsync = async (): Promise<void> => {
    try {
      ...
      // 提交数据
      const msg: string = await ColumnService.edit(renderData.data);
  ...
  (async () => {
    ...
    //写入数据
    try {
      renderData.data = await ColumnService.getEditData(props.id);
